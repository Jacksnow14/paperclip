...
if [[ -f "$HALT_FILE" ]]; then
  ...
  # releases under a halt would fight whoever is recovering it.
  if grep -q "AUR-5095" "$HALT_FILE" 2>/dev/null; then
    # Our own halt from a failover that already paged — do not page again.
