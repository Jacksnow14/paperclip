...
describeEmbeddedPostgres("heartbeat orphaned process recovery", () => {
  ...
  it("leaves a deliberately-blocked orphan blocker at status=blocked with no run spawned", async () => {
    const companyId = randomUUID();
    const creatorAgentId = randomUUID();
    const blockedAssigneeAgentId = randomUUID();
    const blockerIssueId = randomUUID();
    ...
    await db.insert(issues).values([
      {
        id: blockerIssueId,
        companyId,
        title: "Founder gate: upload identity documents",
        // Deliberately blocked and unassigned by a human — no agent can close this.
        status: "blocked",
        priority: "critical",
        createdByAgentId: creatorAgentId,
        issueNumber: 1,
  ...
  it("does not auto-assign an orphan blocker that already has a human assignee", async () => {
    const companyId = randomUUID();
    const creatorAgentId = randomUUID();
