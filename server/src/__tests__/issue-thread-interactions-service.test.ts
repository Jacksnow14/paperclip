...
describeEmbeddedPostgres("issueThreadInteractionService", () => {
  ...
  it("auto-expires an older pending interaction by the same creator even when the replacement uses a different kind", async () => {
    const companyId = randomUUID();
    const goalId = randomUUID();
    const issueId = randomUUID();
    const creatorAgentId = randomUUID();
    ...
    await db.insert(goals).values({
      id: goalId,
      companyId,
      title: "Same-creator replacement across kinds",
      level: "task",
      status: "active",
    });
    await db.insert(agents).values({
    ...
    const first = await interactionsSvc.create({
      ...
      payload: {
        ...
        questions: [
          {
            id: "path",
            prompt: "Which first-mile path should we try?",
            selectionMode: "single",
            options: [
              { id: "a", label: "Path A" },
              { id: "b", label: "Path B" },
    ...
    const second = await interactionsSvc.create({
      ...
      payload: {
        version: 1,
        prompt: "Proceed with the live Shopify step?",
      },
    }, {
      agentId: creatorAgentId,
    });
  ...
  it("replays the AUR-2156 interaction jam and ends with 2 pending instead of 12+ (AUR-3928)", async () => {
    const companyId = randomUUID();
    const goalId = randomUUID();
    const issueId = randomUUID();
    ...
    expect(pending.map((entry) => entry.id).sort()).toEqual(
      [created[16].id, created[17].id].sort(),
    );
  });
});
