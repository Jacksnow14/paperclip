...
describeEmbeddedPostgres("memoryService local basic persistence", () => {
  ...
  // AUR-1423: exact key lookup bypasses fuzzy search
  it("returns records matching the exact title key and excludes partial/fuzzy matches", async () => {
    const companyId = randomUUID();
    await db.insert(companies).values({
      id: companyId,
      name: "Paperclip Key Lookup Test",
      issuePrefix: `K${companyId.replace(/-/g, "").slice(0, 5).toUpperCase()}`,
      requireBoardApprovalForNewAgents: false,
    });
    ...
    const exactTitle = "retrospective/AUR-1234/tool-gaps";
    const prefixTitle = "retrospective/AUR-1234/tool-gaps/extra";

    await service.capture(
      companyId,
      {
        bindingKey: "key-lookup-test",
        scope: { scopeType: "org", scopeId: companyId },
        source: { kind: "manual_note", externalRef: "key-lookup-exact" },
        title: exactTitle,
        content: "Exact key record content.",
        reviewState: "accepted",
      },
      actor,
    );
    await service.capture(
      companyId,
      {
        bindingKey: "key-lookup-test",
        scope: { scopeType: "org", scopeId: companyId },
        source: { kind: "manual_note", externalRef: "key-lookup-prefix" },
        title: prefixTitle,
        content: "Prefix key record content.",
        reviewState: "accepted",
      },
      actor,
    );
    await service.capture(
      companyId,
      {
        bindingKey: "key-lookup-test",
        scope: { scopeType: "org", scopeId: companyId },
        source: { kind: "manual_note", externalRef: "key-lookup-notitle" },
        title: null,
        content: "No title record.",
        reviewState: "accepted",
      },
      actor,
    );

    const results = await service.listRecords(
      companyId,
      { key: exactTitle, limit: 50, includeDeleted: false, includeRevoked: false, includeExpired: false, includeSuperseded: false },
      actor,
    );

    expect(results).toHaveLength(1);
    expect(results[0].title).toBe(exactTitle);
  });

  // AUR-1418: performance scorecard prefix lookup powers CEO routing.
  it("lists performance scorecard records by title prefix and ignores unrelated keys", async () => {
    const companyId = randomUUID();
    await db.insert(companies).values({
      id: companyId,
      name: "Paperclip Scorecard Prefix Test",
    ...
    const binding = await service.createBinding(companyId, {
      key: "scorecard-prefix-test",
      name: "Scorecard prefix test",
      providerKey: "local_basic",
      config: {},
      enabled: true,
    });
    ...
    const matches = [
      "performance/agent-7/feature/2026-05-26",
      "performance/agent-7/feature/2026-05-27",
      "performance/agent-7/feature/2026-05-28",
    ];
    const decoys = [
      "performance/agent-7/bug/2026-05-28",
      "performance/agent-8/feature/2026-05-28",
      "retrospective/AUR-1418/tool-gaps",
    ];

    for (const [index, title] of [...matches, ...decoys].entries()) {
      await service.capture(
        companyId,
        {
          bindingKey: "scorecard-prefix-test",
          scope: { scopeType: "org", scopeId: companyId },
          source: { kind: "manual_note", externalRef: `scorecard-${index}` },
          title,
          content: title,
    ...
    const results = await service.listRecords(
      companyId,
      {
        titlePrefix: "performance/agent-7/feature/",
        limit: 50,
        includeDeleted: false,
