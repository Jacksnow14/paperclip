...
// Ported onto master for AUR-3523/AUR-3532 (the original branches were never
// merged); see docs/gmail-io.md for the current enforcement wiring.
//
...
function recipients(input: GmailOutboundCheckInput): string[] {
  const raw = [input.to, ...toArray(input.cc)].join(" ");
  return raw
    .split(/[,;\s]+/)
 ...
 * fraud/abuse/legal/chargeback/law-enforcement report (or a blocklisted
 * recipient domain) and requires an explicit CEO-approved approval ID before
