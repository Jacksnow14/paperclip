...
export function memoryService(
  ...
  // Structured retrospective categories that carry no sensitive content and
  ...
  const AUTO_ACCEPT_CATEGORIES = new Set(["performance_scorecard", "tool_gap", "lesson", "routing_rationale", "synthesis", "scorecard_adjusted", "roi_ledger"]);
  ...
  function buildLocalBasicConditions(companyId: string, filters: MemoryListRecordsQuery, actor: ActorInfo) {
    ...
    if (filters.key) conditions.push(eq(memoryLocalRecords.title, filters.key));
    if (filters.titlePrefix) {
      const escaped = filters.titlePrefix.replace(/[\\%_]/g, "\\$&");
      conditions.push(ilike(memoryLocalRecords.title, `${escaped}%`));
    }
    if (filters.q) {
