...
export function parseRateLimitResetTime(
  ...
  const clockMatch =
    lower.match(/try again at\s+(\d{1,2}):(\d{2})(?:\s*(am|pm))?(?:\s*\(?\s*([a-z+\-0-9_/]+))?/) ??
    lower.match(/resets?\s+(\d{1,2}):(\d{2})(?:\s*(am|pm))?(?:\s*\(?\s*([a-z+\-0-9_/]+))?/) ??
    lower.match(/resets?\s+(\d{1,2})()(am|pm)(?:\s*\(?\s*([a-z+\-0-9_/]+))?/);
  if (clockMatch) {
...
export function computeBackoffRetryAfter(retryCount: number, now: Date = new Date()): Date {
  ...
  const delayMs = Math.min(BACKOFF_CAP_MS, BACKOFF_BASE_MS * 2 ** safeCount);
