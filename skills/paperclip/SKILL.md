...
**Hairpin-NAT gotcha (same-VPS agents/scripts):** `PAPERCLIP_API_URL` is often configured as the control plane's public IP. If your script/agent runs on that same VPS, calls to the public IP can hairpin-NAT and hang instead of erroring. Prefer `127.0.0.1` (never `localhost` — IPv6 `::1` resolution hangs) over the public IP when both resolve to the same host. Do not just hard-code `127.0.0.1` blindly though — non-local adapters and remote operators legitimately need the configured URL.

- In `.mjs` scripts under `scripts/`, use the shared resolver instead of reading `process.env.PAPERCLIP_API_URL` directly: `import { resolveApiBase } from './lib/paperclip-api-base.mjs'; const API_URL = await resolveApiBase();`. It tries the configured URL first (with a short probe) and falls back to a `127.0.0.1` loopback swap (same scheme/port/path) only if that probe fails, memoizing the result for the process lifetime and logging which base won via `console.error('[api-base] ...')`.
- For ad-hoc `curl`/`python urllib` one-liners (not inside a `.mjs` script), apply the same fallback manually if the configured host hangs: swap only the hostname to `127.0.0.1`, e.g. `curl --max-time 5 "http://127.0.0.1:${PAPERCLIP_API_URL##*:}/api/health"` (or construct the loopback URL by hand, preserving scheme/port/path) rather than switching to `localhost`.
- There is no `scripts/pcp` CLI in this repo — don't invent one; use the resolver above for `.mjs` scripts or the manual fallback for ad-hoc shell/python calls.
